...
namespace CDAService.Utils
{
    public static class RSA256TokenUtils
    {
        public class RQPTokenManager
        {
            ...
            public string GenerateToken()
            {
                ...
                var tokenDescriptor = new SecurityTokenDescriptor
                {
                    Subject = new ClaimsIdentity(new[] 
                                    { 
                                        new Claim(JwtRegisteredClaimNames.Sub, $"{_userSessionId}@{_iss}"),
            ...
            public bool ValidateToken(string token, out RQPModel rqpModel)
            {
                var rsa = RSA.Create();
                rsa.ImportFromPem(LoadGeneratedRsaPrivateKeyPem());
            ...
            public string LoadGeneratedRsaPrivateKeyPem()
            {
