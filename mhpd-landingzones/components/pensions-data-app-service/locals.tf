locals {
  ...
  app_name          = "pension-data-service-${local.loc}-${var.env}"
  service_plan_name = "${var.product}-asp-pension-data-svc-${local.loc}-${var.env}"
  ...
  pension_data_app_settings = {
    ...
    "PensionRetrievalServiceUrl"                            = "https://func-pensions-retrieval-${local.loc}-${var.env}.azurewebsites.net/"
    "RetrievedPensionsServiceUrl"                           = "https://func-retrieved-pensions-${local.loc}-${var.env}.azurewebsites.net/"
    "tokenIntegrationServiceUrl"                            = "https://token-integration-service-${local.loc}-${var.env}.azurewebsites.net/"
    "MapsCdaServiceUrl"                                     = "https://cda-service-${local.loc}-${var.env}.azurewebsites.net/"
    "OpenApiServerUrl"                                      = "https://pension-data-service-${local.loc}-${var.env}.azurewebsites.net/"
