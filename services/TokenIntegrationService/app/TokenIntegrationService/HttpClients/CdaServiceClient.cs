...
public class CdaServiceClient(IHttpClientFactory httpClientFactory, IConfiguration configuration, ILogger<CdaServiceClient> logger) : ICdaServiceClient
{
    public async Task<RptsModel> PostRptAsync(CdaTokenRequestModel request, RequestHeaderModel requestHeader)
    {
        try
        {
            ...
            client.DefaultRequestHeaders.Add(HeaderConstants.RequestId, requestHeader.XRequestId);
            ...
            var response = await client.PostAsync(UrlHelper.ConstructEndPoint(request, HttpEndpoints.CdaTokenServiceEndpoint), null);
